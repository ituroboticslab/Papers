...
In order to extract ZMs in local manner, the input image is divided by $k\times k$-sized subimages in which $k$ is a predefined partitioning parameter. If the image is not divided by $k$ perfectly, the rest of the part is simply ignored. 
...
\begin{bmatrix}
\mathbf{Z}(I_{11}) & \hdots & \mathbf{Z}(I_{1Q}) \\
\vdots & \ddots & \vdots \\
\mathbf{Z}(I_{P1}) & \hdots & \mathbf{Z}(I_{PQ})
 \end{bmatrix}
 \longrightarrow
 \begin{bmatrix}
Z_{00}(I_{11}) & \hdots & Z_{nm}(I_{11}) & \hdots & Z_{00}(I_{1Q}) & \hdots & Z_{nm}(I_{1Q}) \\
\vdots & \ddots & \vdots & & \vdots & \ddots & \vdots  \\
Z_{00}(I_{P1}) & \hdots & Z_{nm}(I_{P1}) & \hdots & Z_{00}(I_{PQ}) & \hdots & Z_{nm}(I_{PQ}) \\
 \end{bmatrix},
 \label{eq:lzm_op}
\end{equation}
...
\mathbf{Z} ( \mathbf{I}_{p\times q} )
 \longrightarrow
 \begin{bmatrix}
a_{00}^{11} + \mathbf{i}b_{00}^{11} & \hdots & a_{nm}^{11} + \mathbf{i}b_{nm}^{11} & \hdots &  a_{00}^{1Q} + \mathbf{i}b_{00}^{1Q} & \hdots & a_{nm}^{1Q} + \mathbf{i}b_{nm}^{1Q} \\
\vdots & \ddots & \vdots & & \vdots & \ddots & \vdots  \\
a_{00}^{P1} + \mathbf{i}b_{00}^{P1} & \hdots & a_{nm}^{P1} + \mathbf{i}b_{nm}^{P1} & \hdots & a_{00}^{PQ} + \mathbf{i}b_{00}^{PQ} & \hdots & a_{nm}^{PQ} + \mathbf{i}b_{nm}^{PQ} \\
 \end{bmatrix}.
\end{equation}

Therefore, $k\times kK(n)$-sized complex coefficient matrix is obtained. In order to facilitate the representation of the data and to reduce the size of the descriptor vector, a coarse quantization takes place at this step. First, the complex numbers are decoupled as real and imaginary components, then the sign of each matrix element is taken to binarize the decoupled $k\times 2kK(n)$-sized matrix as follows:

\begin{equation}
\mathbf{Z} ( \mathbf{I}_{p\times q} )
 \longrightarrow
 \begin{bmatrix}
a_{00}^{11} & b_{00}^{11} & \hdots & a_{nm}^{11} & b_{nm}^{11} & \hdots &  a_{00}^{1Q} & b_{00}^{1Q} & \hdots & a_{nm}^{1Q} & b_{nm}^{1Q} \\
\vdots & \vdots & \ddots & \vdots & \vdots &  & \vdots & \vdots & \ddots & \vdots & \vdots \\
a_{00}^{P1} & b_{00}^{P1} & \hdots & a_{nm}^{P1} & b_{nm}^{P1} & \hdots &  a_{00}^{PQ} & b_{00}^{PQ} & \hdots & a_{nm}^{PQ} & b_{nm}^{PQ}
 \end{bmatrix}
  \label{eq:lzm_decoupled}
\end{equation}
...
\mathbf{Z} ( \mathbf{I}_{p\times q} )
 \longrightarrow
 \begin{bmatrix}
s(a_{00}^{11}) & s(b_{00}^{11}) & \hdots & s(a_{nm}^{11}) & s(b_{nm}^{11}) & \hdots & s(a_{00}^{1Q}) & s(b_{00}^{1Q}) & \hdots & s(a_{nm}^{1Q}) & s(b_{nm}^{1Q}) \\
\vdots & \vdots & \ddots & \vdots & \vdots & & \vdots & \vdots & \ddots & \vdots & \vdots \\
s(a_{00}^{P1}) & s(b_{00}^{P1}) & \hdots & s(a_{nm}^{P1}) & s(b_{nm}^{P1}) & \hdots &  s(a_{00}^{PQ}) & s(b_{00}^{PQ}) & \hdots & s(a_{nm}^{PQ}) & s(b_{nm}^{PQ})
 \end{bmatrix}
  \label{eq:lzm_sign}
\end{equation}
where $s(\bullet) \equiv sgn(\bullet)$ which is the signum function. According to (\ref{eq:global_zernike}), $Z_{nm}$ with $m = 0$ is omitted since their imaginary components turn out to be constantly equal to zero. 

The next step is representing the binary matrix shown in (\ref{eq:lzm_sign}) with histograms. Using the binary values for the overall image representation is not a practical solution. Describing binary data with histograms are more common solution to obtain the image representation. As the methodology described so far, $k\times 2kK(n)$-sized binary matrix is extracted. Each row of this matrix is linearly combined by weighting each of them as a power of $2$. Hence the final feature vector $\mathbf{X}$ is obtained as follows:
