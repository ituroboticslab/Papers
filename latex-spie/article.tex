...
\begin{figure}
        ...
        \caption{Comparison between datasets when both grayscale and depth images are utilized.}
        \label{subfig:pr_all}
        \end{subfigure}

        \hspace{-3mm}
        \caption{Precision vs. Recall curves of the results.}
        \label{fig:pr_curves}
\end{figure}
...
In conclusion, this paper presents a novel image description technique\footnote{Readers can access the source code from the following link: \emph{https://github.com/ituroboticslab/Quantized-Local-Zernike-Moments}} that is used to achieve loop closure detection within a SLAM framework. The technique works quite rapidly and operates in real-time. Moreover, it results successful loop closing on different lightning conditions by using the depth information.
